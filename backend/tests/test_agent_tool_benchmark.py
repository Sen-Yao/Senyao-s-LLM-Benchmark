...
def test_agent_skill_routing_hermes_yaml_uses_granular_scoring():
    ...
    response_full = (
        "使用 `hermes config set providers.custom.foo.api_base <url>` 配置 endpoint；"
        "使用 `hermes config set providers.custom.foo.api_key <key>` 配置密钥；"
        "再用 `hermes config set model.provider custom:foo` 和 `hermes config set model.model gpt-x` 切换模型；"
        "最后用 `hermes config get` 验证。"
    )
    response_no_verify = response_full.replace("最后用 `hermes config get` 验证。", "")
    response_minimal = "使用 `hermes config set model.provider custom:foo` 配置 provider。"

    full = AgentStateMachineEvaluator().evaluate(response_full, {**config, "trace": loaded_trace})
    no_verify = AgentStateMachineEvaluator().evaluate(response_no_verify, {**config, "trace": loaded_trace})
    minimal = AgentStateMachineEvaluator().evaluate(response_minimal, {**config, "trace": loaded_trace})
